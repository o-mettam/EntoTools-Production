...
- `public/theme.js` — **single source of truth for the theme**: brand `lime`/`slate` palette (Tailwind config), dark-mode preference storage/migration, and the shared `toggleTheme()` / `updateThemeToggleUI()` used by every page's Settings switch.
- `public/theme.css` — shared base typography, common components (`.fade-in`, `.hover-lift`, `.loader`, `.tab-active`, `.settings-panel`) and every dark-mode override.
...
`public/theme.js`, `public/theme.css`, `public/feedback.js`, `public/sync/*.js`
and `public/stations.json` are *not* generated by the build script — edit those
files in `public/` directly.

To change a color or a dark-mode rule site-wide, edit `public/theme.js` /
`public/theme.css`. Page `<style>` blocks should only contain rules unique to
that page.
